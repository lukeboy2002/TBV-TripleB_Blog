...
{{--                                <img src="{{ asset('storage/' . $member->member->image) }}" alt="{{ $member->username }}" class="h-10 w-auto" >--}}
                            ...
                            <td class="px-6 py-4 flex justify-end space-x-4">
                                @if ($member->trashed())
                                    @role('admin')
                                    <x-link.primary href="{{ route('admin.users.trashed.restore' , $member->id) }}" class="px-2.5 py-2.5 text-xs font-medium">Restore</x-link.primary>
                                    <x-link.btn-danger href="{{ route('admin.users.trashed.destroy' , $member->id) }}" class="px-2.5 py-2.5 text-xs font-medium">Force Delete</x-link.btn-danger>
                                    @endrole
                                    @role('member')
